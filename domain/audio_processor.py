...
class AudioProcessor:
    ...
    def process_chunks_parallel(self, data, chunk_size, process_func, progress_callback=None, **kwargs):
        ...
        for i, (start, end) in enumerate(chunk_indices):
            ...
            if i < total_chunks - 1 and overlap > 0:
                window[-overlap:] = np.linspace(1, 0, overlap)
            # Ajustăm dacă depășim limita semnalului
            write_len = min(chunk_len, len(result) - start)
            if write_len > 0:
                result[start:start + write_len] += chunk[:write_len] * window[:write_len]
                weight[start:start + write_len] += window[:write_len]
